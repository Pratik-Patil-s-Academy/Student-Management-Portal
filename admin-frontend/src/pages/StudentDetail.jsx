...
import { getStudentById, updateStudent, updateStudentStatus, deleteStudent, assignBatch } from '../services/studentService';
import { getAllBatches } from '../services/batchService';
import { FaArrowLeft, FaUserGraduate, FaEdit, FaSave, FaTimes, FaTrash, FaPhone, FaCalendarAlt, FaIdCard, FaUsers } from 'react-icons/fa';
...
const StudentDetail = () => {
    ...
    const handleOpenBatchModal = async () => {
        try {
            ...
            toast.error('Failed to fetch batches: ' + (err.message || 'Unknown error'));
        }
    };
    ...
    const handleAssignBatch = async () => {
        ...
        if (!window.confirm('Assign this batch to the student?')) return;

        setAssigningBatch(true);
        try {
            const result = await assignBatch(id, selectedBatchId);
            if (result.success) {
                // Find selected batch details for immediate UI update
                const assignedBatch = availableBatches.find(b => b._id === selectedBatchId);

                // Update student state with the new batch info
                // If backend returns populated student, use it. Otherwise patch it manually.
                ...
                <div className="flex justify-between items-center">
                    <div className="flex">
                        ...
                        <div className="ml-3">
                            <p className={`text-sm ${student.status === 'Admitted' ? 'text-green-700' : 'text-red-700'}`}>
                                Academic Year Status: <strong>{student.status}</strong> | Batch: <strong>{student.batch?.name || 'Unassigned'}</strong>
                            </p>
                        </div>
                    </div>
                    {(!student.batch || !student.batch.name) && (
                        <button
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white hover:bg-blue-700 font-semibold rounded-lg transition-all shadow-md hover:shadow-lg text-sm"
                        >
                    ...
                    <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6 animate-fadeIn">
                        ...
                        <div className="mb-6">
                            ...
                            <select
                                ...
                                className="w-full px-4 py-3 border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#2C3E50] focus:border-transparent"
                                disabled={assigningBatch}
                            >
                                <option value="">-- Choose a batch --</option>
                                {availableBatches.map((batch) => (
                                    <option key={batch._id} value={batch._id}>
                                        {batch.name} ({batch.standard}) - {batch.timing}
                                    </option>
                                ))}
                            </select>
                            {availableBatches.length === 0 && (
                                <p className="text-sm text-gray-500 mt-2">No batches available</p>
                            )}
                        </div>
                        ...
                        <div className="flex gap-3">
                            <button
                                ...
                                className="flex-1 px-4 py-2 bg-gray-200 text-gray-700 hover:bg-gray-300 font-semibold rounded-lg transition-all"
                                disabled={assigningBatch}
                            >
                                Cancel
                            </button>
                            <button
                                ...
                                className="flex-1 px-4 py-2 bg-[#2C3E50] text-white hover:bg-[#34495E] font-semibold rounded-lg transition-all shadow-md hover:shadow-lg disabled:opacity-50"
